...
def test_alternate_storage(admin_client, settings):
    ...
    with patch('django.db.models.fields.files.default_storage._wrapped', storage):
        ...
        assert storage.filesystem.exists('images/{}/animated.gif'.format(image_id))
        assert storage.filesystem.exists('images/{}/optimized.gif'.format(image_id))

        # Delete Image
        resp = admin_client.post("/images/api/{0}".format(image_id),
                                 REQUEST_METHOD="DELETE")

        assert not storage.filesystem.exists('images/{}/animated.gif'.format(image_id))
        assert not storage.filesystem.exists('images/{}/optimized.gif'.format(image_id))
